...
#ifndef INPUT_EVENTS_ADAFRUIT_RESISTIVE_TOUCH_SCREEN_ADAPTER_H
#define INPUT_EVENTS_ADAFRUIT_RESISTIVE_TOUCH_SCREEN_ADAPTER_H
...
#include "BaseTouchScreenAdapter.h"


namespace input_events {

/**
 * @brief Concrete implementation of BaseTouchScreenAdapter for Adafruit's resistive TouchScreen class.
 * 
 ...
 * 
 */
class AdafruitResistiveTouchScreenAdapter : public BaseTouchScreenAdapter {
...
public:
     ...
     */
//    AdafruitResistiveTouchScreenAdapter(uint8_t pinXPos, uint8_t pinYPos, uint8_t pinXNeg,uint8_t pinYNeg, uint16_t ohms=300, uint16_t displayWidth=240, uint16_t displayHeight=320 ) 
    // AdafruitResistiveTouchScreenAdapter(TouchScreen& touchScreen, uint16_t displayWidth=240, uint16_t displayHeight=320 ) :
    ...
    AdafruitResistiveTouchScreenAdapter(uint8_t pinXPos, uint8_t pinYPos, uint8_t pinXNeg,uint8_t pinYNeg, uint16_t ohms=300, uint16_t displayWidth=240, uint16_t displayHeight=320 ) :
        touchscreen(pinXPos, pinYPos, pinXNeg, pinYNeg, ohms)
        {   
            nativeDisplayWidth = displayWidth;
            nativeDisplayHeight = displayHeight;
            this->displayHeight = nativeDisplayHeight;
...
//        delete touchscreen; //Safe to delete even if null;
    ...
    /**
     * @brief Get the TouchPoint_s struct
     * 
     * @details The TouchPoint_s is populated with X and Y mapped from the raw resistance values to display pixel position.
     * 
     * @return TouchPoint_s 
     */
    TouchPoint_s getTouchPoint(void) override { 
    ...
    /**
     * @brief Get the raw TouchPoint_s struct
     * 
     * @details The TouchPoint_s is populated with the raw resistance values from the Adafruit TouchScreen library.
     * 
     * Use these values to measure and set the raw min/max X and Y values.
     * 
     * @return TouchPoint_s 
     */
    TouchPoint_s getTouchPointRaw(void) {
        TSPoint tsPoint = touchscreen.getPoint();
        TouchPoint_s touchpoint;
